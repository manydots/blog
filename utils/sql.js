var sysUser = {
	...
	updateEditById: 'UPDATE article SET title = ? , context = ? , tags = ? , modify_time = ? WHERE id = ?',
	getArticleById: "SELECT a.id,a.author,a.state,a.user_id,a.title,a.context,a.tags,a.creat_time,a.modify_time FROM article a WHERE id = ? AND state = ?",
	getArticleOwner: 'SELECT a.id,a.state,a.user_id FROM article a WHERE id = ? AND user_id = ?',
	getArticleByUserId: "SELECT a.id,a.author,a.state,a.user_id,a.title,a.context,a.tags,a.creat_time,a.modify_time FROM article a WHERE user_id = ? AND state = ? order by creat_time desc limit ?,?",
